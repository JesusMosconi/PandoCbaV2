...
router.post("/colors", ...requerirAdmin, asyncHandler(async (req, res) => {
...
router.delete("/colors/:id", ...requerirAdmin, asyncHandler(async (req, res, next) => {
  try {
...
router.put("/colors/:id", ...requerirAdmin, asyncHandler(async (req, res) => {
  const error = validateBody(req.body, requiredFields);
  if (error) return res.status(400).json({ message: error });
